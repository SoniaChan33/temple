import {
  Blockhash,
  createSolanaClient,
  createTransaction,
  generateKeyPairSigner,
  Instruction,
  isSolanaError,
  KeyPairSigner,
  signTransactionMessageWithSigners,
} from 'gill'
import {
  fetchTemple,
  getCloseInstruction,
  getDecrementInstruction,
  getIncrementInstruction,
  getInitializeInstruction,
  getSetInstruction,
  getInitUserInstruction,
  fetchUserState,
} from '../src/client/js'
// @ts-ignore error TS2307 suggest setting `moduleResolution` but this is already configured
import { loadKeypairSignerFromFile } from 'gill/node'

const { rpc, sendAndConfirmTransaction } = createSolanaClient({ urlOrMoniker: process.env.ANCHOR_PROVIDER_URL! })

describe('temple', () => {
  let payer: KeyPairSigner
  let temple: KeyPairSigner

  beforeAll(async () => {
    temple = await generateKeyPairSigner()
    payer = await loadKeypairSignerFromFile(process.env.ANCHOR_WALLET!)
  })

  it('Initialize Temple', async () => {
    // ARRANGE
    expect.assertions(1)
    const ix = getInitializeInstruction({ payer: payer, temple: temple })

    // ACT
    await sendAndConfirm({ ix, payer })

    // ASSER
    const currentTemple = await fetchTemple(rpc, temple.address)
    expect(currentTemple.data.count).toEqual(0)
  })

  it('Increment Temple', async () => {
    // ARRANGE
    expect.assertions(1)
    const ix = getIncrementInstruction({
      temple: temple.address,
    })

    // ACT
    await sendAndConfirm({ ix, payer })

    // ASSERT
    const currentCount = await fetchTemple(rpc, temple.address)
    expect(currentCount.data.count).toEqual(1)
  })

  it('Increment Temple Again', async () => {
    // ARRANGE
    expect.assertions(1)
    const ix = getIncrementInstruction({ temple: temple.address })

    // ACT
    await sendAndConfirm({ ix, payer })

    // ASSERT
    const currentCount = await fetchTemple(rpc, temple.address)
    expect(currentCount.data.count).toEqual(2)
  })

  it('Decrement Temple', async () => {
    // ARRANGE
    expect.assertions(1)
    const ix = getDecrementInstruction({
      temple: temple.address,
    })

    // ACT
    await sendAndConfirm({ ix, payer })

    // ASSERT
    const currentCount = await fetchTemple(rpc, temple.address)
    expect(currentCount.data.count).toEqual(1)
  })

  it('Set temple value', async () => {
    // ARRANGE
    expect.assertions(1)
    const ix = getSetInstruction({ temple: temple.address, value: 42 })

    // ACT
    await sendAndConfirm({ ix, payer })

    // ASSERT
    const currentCount = await fetchTemple(rpc, temple.address)
    expect(currentCount.data.count).toEqual(42)
  })

  it('Set close the temple account', async () => {
    // ARRANGE
    expect.assertions(1)
    const ix = getCloseInstruction({
      payer: payer,
      temple: temple.address,
    })

    // ACT
    await sendAndConfirm({ ix, payer })

    // ASSERT
    try {
      await fetchTemple(rpc, temple.address)
    } catch (e) {
      if (!isSolanaError(e)) {
        throw new Error(`Unexpected error: ${e}`)
      }
      expect(e.message).toEqual(`Account not found at address: ${temple.address}`)
    }
  })

  it('Initialize User State', async () => {
    // ARRANGE
    expect.assertions(4)
    const ix = await getInitUserInstruction({ user: payer })

    // ACT
    await sendAndConfirm({ ix, payer })

    // ASSERT
    const userState = await fetchUserState(rpc, ix.accounts[0].address)
    expect(userState.data.user).toEqual(payer.address)
    expect(userState.data.incensePoints).toEqual(0n)
    expect(userState.data.merit).toEqual(0n)
    expect(userState.data.incenseBalance).toEqual([])
  })
})

// Helper function to keep the tests DRY
let latestBlockhash: Awaited<ReturnType<typeof getLatestBlockhash>> | undefined
async function getLatestBlockhash(): Promise<Readonly<{ blockhash: Blockhash; lastValidBlockHeight: bigint }>> {
  if (latestBlockhash) {
    return latestBlockhash
  }
  return await rpc
    .getLatestBlockhash()
    .send()
    .then(({ value }) => value)
}
async function sendAndConfirm({ ix, payer }: { ix: Instruction; payer: KeyPairSigner }) {
  const tx = createTransaction({
    feePayer: payer,
    instructions: [ix],
    version: 'legacy',
    latestBlockhash: await getLatestBlockhash(),
  })
  const signedTransaction = await signTransactionMessageWithSigners(tx)
  return await sendAndConfirmTransaction(signedTransaction)
}
